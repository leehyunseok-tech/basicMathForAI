...
### log 기호는 "몇 제곱하면 ○가 될까"를 묻는 것

기호가 낯설 뿐, 하는 일은 **질문 하나**입니다.

![로그 상자 표기](../assets/fig07-4b-logboxes.svg)

$$
\log_2 \bigcirc = (2\text{를 몇 제곱하면 } \bigcirc \text{가 될까})
$$

- 아래 작은 숫자 = **무엇을** 곱할지 (밑)
- 안쪽 숫자 = **얼마로** 만들고 싶은지 (알고 싶은 수)

밑이 바뀌어도 규칙은 같습니다.

| 식 | 읽으면 |
|---|---|
| $\log_{10} 100 = 2$ | 10을 2제곱하면 100 |
| $\log_3 9 = 2$ | 3을 2제곱하면 9 |
| $\log_5 125 = 3$ | 5를 3제곱하면 125 |
...
$y = \log_2 x$ 나 $y = \log_{10} x$ 는 로그함수지만, $y = x^2$ 은 이 형태가 아니므로 로그함수가 아닙니다.

그래프는 지수함수를 **대각선으로 뒤집은** 모양이고, **오른쪽 위로 완만하게 증가**합니다.

![로그함수 그래프](../assets/fig07-4c-loggraph.svg)
...
x가 **10배**가 되어야 y가 겨우 **1** 오릅니다.
이 "큰 걸 작게" 성질이 실무에서 계속 쓰입니다.

---

## 7.7-1 로그가 답해 주는 질문: "몇 년이 걸릴까?"

로그가 실제로 쓰이는 가장 알기 쉬운 예입니다.

> **1000만 원을 연이율 10%(매년 1.1배)로 굴리면, 1억 원이 되는 건 몇 년 후일까요?**

![투자 예시](../assets/fig07-4d-investment.svg)
...
|---|---|
| 처음 | 1,000만 원 |
| 1년 후 | $1000 \times 1.1 = $ **1,100만 원** |
| 2년 후 | $1100 \times 1.1 = $ **1,210만 원** |
| 3년 후 | $1210 \times 1.1 = $ **1,331만 원** |

x년 후의 투자금은 처음의 $1.1^x$ 배입니다. 10배가 되는 시점을 찾으려면:

> **"1.1을 몇 제곱하면 10이 될까?"** → $\log_{1.1} 10$

계산기로 두드리면 **24.15…** 가 나옵니다. 즉 **25년**이면 10배가 됩니다.

> 🎯 **주목할 점**
> 이 질문은 지수로는 답할 수 없습니다. $1.1^x = 10$ 에서 **x를 끄집어내야** 하니까요.
> 그걸 해 주는 게 로그입니다.
>
> AI에서도 똑같은 형태로 등장합니다.
> *"손실을 10분의 1로 줄이려면 몇 epoch가 더 필요할까?"* — 같은 계산입니다.
